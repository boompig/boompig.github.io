		...
		while (effect !== null) {
			...
			if (!skip && effect.fn !== null) {
				if (is_branch) effect.f ^= CLEAN;
					...
					if ((effect.f & 16) !== 0) target.block_effects.push(effect);
...
function update_derived(derived) {
	...
	if (batch_values !== null) {
		if (effect_tracking()) batch_values.set(derived, value);
...
function destroy_effect(effect, remove_dom = true) {
	var removed = false;
	if ((remove_dom || (effect.f & 262144) !== 0) && effect.nodes_start !== null && effect.nodes_end !== null) {
		remove_effect_dom(effect.nodes_start, effect.nodes_end);
		removed = true;
	}
...
function update_reaction(reaction) {
	...
	try {
		...
		if (new_deps !== null) {
			...
			if (is_updating_effect && effect_tracking() && (reaction.f & 512) !== 0) for (i = skipped_deps; i < deps.length; i++) (deps[i].reactions ??= []).push(reaction);
...
function update_effect(effect) {
	...
	try {
		if ((flags$1 & 16) !== 0) destroy_block_effect_children(effect);
...
export { async_mode_flag as C, escape_html as S, run as T, getAbortSignal as _, head as a, asClassComponent as b, spread_props as c, getAllContexts as d, getContext as f, lifecycle_function_unavailable as g, ssr_context as h, ensure_array_like as i, stringify as l, setContext as m, bind_props as n, render as o, hasContext as p, element as r, slot as s, attr_class as t, createContext as u, readable as v, noop as w, attr as x, writable as y };
